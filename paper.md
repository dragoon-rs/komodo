...
We present **Komodo**, a library that allows one to (**1**) encode data with forward erasure-code (FEC) techniques such as Reed-Solomon encoding, (**2**) prove the resulting _shards_ with cryptographic protocols, (**3**) verify their validity on the other end of any distributed network, and (**4**) decode the original data from a subset of said _shards_ [@stevan2024performance; @stevan2023assessing].
The library is implemented in the _Rust_ programming language and available under the MIT licenseon the ISAE-SUPAERO GitLab instance[^1] with a mirror on GitHub[^2].
...
The verification mechanisms provided by **Komodo** can help in implementing verifiable information
dispersal or data availability schemes in a distributed storage system or in a blockchain.

For instance, in a distributed storage system, nodes can use **Komodo** to encode incoming data
...
- The `arkworks` ecosystem [@arkworks] is probably the closest library, providing many of the
  ...
  with elliptic curves like BLS12-381 or BN254 among many others, and linear algebra operations like
...
- **Komodo** also adds a unified benchmarking API, allowing to compare different combinations of
  ...
  [@stevan2024performance; @stevan2023assessing]. Finally, a modular design allows one to extend
...
The time to run `commit`, `prove`m and `verify` has been measured for $k = 8$ and
...
All raw results have been pushed to a separate dedicated repository[^3] and all the pre-compiled figures are available on the _GitLab pages_ instance of ISAE SUPAERO[^4].
...
[^3]: results repository: [https://gitlab.isae-supaero.fr/dragoon/komodo-benchmark-results](https://gitlab.isae-supaero.fr/dragoon/komodo-benchmark-results)
[^4]: pre-compiled results figures: [https://gitlab-pages.isae-supaero.fr/dragoon/komodo-benchmark-results/](https://gitlab-pages.isae-supaero.fr/dragoon/komodo-benchmark-results/)
